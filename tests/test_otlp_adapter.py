...
def test_nanosecond_timestamps_are_converted_with_exact_integer_math():
    """Microsecond rendering must not depend on float64 having enough mantissa left.

    The conversion rounds nanoseconds to the nearest microsecond. It agrees with the old
    float path at today's epochs and, unlike it, keeps doing so indefinitely.
    """
    base = 1752451200_000_000_000
    document = {
        "resourceSpans": [
            {
                "scopeSpans": [
                        ...
                        "spans": [
                            {
                                ...
                                "spanId": "a",
                                "name": "first",
                                "startTimeUnixNano": str(base + 1_000),
                            },
                            {
                                "traceId": "t1",
                                "spanId": "b",
                                "parentSpanId": "a",
                                "name": "second",
                                "startTimeUnixNano": str(base + 123_400),
                            },
                        ]
                    }
                ]
    ...
    raw = OTLPSpanAdapter(document).ingest("t1")
    stamps = [step.ts for step in raw.steps]
    assert stamps[0].endswith("00:00.000001+00:00"), stamps[0]
    assert stamps[1].endswith("00:00.000123+00:00"), stamps[1]
