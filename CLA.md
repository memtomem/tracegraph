# Tracegraph Individual Contributor License Agreement

Thank you for your interest in contributing to Tracegraph, a project maintained
by **DAPADA Inc.** (주식회사 다파다, contact@dapada.co.kr) and the Tracegraph
contributors.
...
and future Contributions submitted to Tracegraph. Except for the license
granted herein to DAPADA and recipients of software distributed by DAPADA,
You reserve all right, title, and interest in and to Your Contributions.

**Scope.** This Agreement covers Contributions to the Tracegraph repository
...
Tracegraph project at `github.com/memtomem/tracegraph` (the "Work"). For the
...
Your signature is stored in `signatures/v1/cla.json` on this repository's
`cla-signatures` branch. You only need to sign once per GitHub account per
repository. Tracegraph and memtomem keep independent signature stores, so
contributors who open pull requests against both projects sign in each one
(still one-time per account).
